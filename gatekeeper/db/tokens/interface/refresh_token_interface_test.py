...
def test_get_latest_refresh_token_for_user(current_time, user):
    ...
    refresh_token_1 = RefreshToken.objects.create(user=user, token_id="t1", token_payload={
        "sub": user.user_id,
    ...
    refresh_token_2 = RefreshToken.objects.create(user=user, token_id="t2", token_payload={
        "sub": user.user_id,
